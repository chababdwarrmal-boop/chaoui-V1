...
async function renderWallet() {
  ...
  shop.innerHTML=(rewards||[]).map(r=>{const ok=Number(currentProfile?.level||1)>=Number(r.level_required||1)&&Number(currentProfile?.coins||0)>=Number(r.cost||0)&&!red.has(r.id);return `<article class="shop-card ${ok?'ready':''}"><div class="shop-icon">${escapeHTML(r.icon||'🎁')}</div><div><h3>${escapeHTML(r.name)}</h3><p>${escapeHTML(r.description||'مكافأة CHAoui')}</p><small>Level ${Number(r.level_required)} · 🪙 ${Number(r.cost)}</small></div><button class="secondary-btn" ${ok?`onclick="redeemReward('${r.id}')"`:"disabled"}>${red.has(r.id)?'مستلمة':Number(currentProfile?.level||1)<Number(r.level_required)?'🔒 Level':Number(currentProfile?.coins||0)<Number(r.cost)?'🪙 ناقص':'شراء'}</button></article>`}).join("")||`<div class="empty-card">المتجر خالي.</div>`;
